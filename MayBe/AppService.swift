...
class AppService: NSObject {
    ...
    func registerAppService(application: UIApplication,launchOptions: [UIApplication.LaunchOptionsKey: Any]?){
        ...
        Bugly.start(withAppId: "bee016a564")
        ...
        ///监听网络权限
        mintorCellularDataRestrictionDidUpdateNotifie()
        
        ///设置字体适配大小
//        SwiftyFitsize.reference(width: 375, iPadFitMultiple: 0.5)
        
        //wcdb
        LxWCDBManger.shared.createTable(table: kTABLE.diaiogue, of: DiaiogueModel.self)
...
//            }, failture: nil)
   ...
   func mintorCellularDataRestrictionDidUpdateNotifie(){
       
       let cellularData = CTCellularData()
       cellularData.cellularDataRestrictionDidUpdateNotifier = {
           (restrictedState) in
        
        NotificationCenter.post(name: NotificationName.RestrictedStateNotice,userInfo: ["state":restrictedState])
