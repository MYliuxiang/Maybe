...
extension String {
    ...
    ///   - font: 字体
    /// - Returns: 计算出的尺寸
    public func ex_attrisize(with size: CGSize, font: UIFont, kern:Int) -> CGSize {
        if isEmpty {
            return .zero
        }
    ...
    /// - Returns: 计算出的宽度
    public func ex_kernWidth(with font: UIFont,kern:Int) -> CGFloat {
        let size = CGSize(width: CGFloat.greatestFiniteMagnitude, height: 0)
        return ex_attrisize(with: size, font: font, kern: kern).width - CGFloat(kern)
    }
...
extension String
{
    ...
    func conversionMillisecond()->Double{
        
        let array = self.split(separator: ":").map{"\($0)"}
        if array.count != 4 {
            return 0.0
        }else{
    
            let one = Double(array[3]).flatMap { $0 / 25 } ?? 0
            let two = Double(array[2]).flatMap { $0} ?? 0
            let three = Double(array[1]).flatMap { $0*60} ?? 0
            let four = Double(array[0]).flatMap { $0*60*60} ?? 0
